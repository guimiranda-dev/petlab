...
import PDFFile from '../PDFExam/coproparasitologico';
...
const PdfCoproparasitologicoView = memo(({ values }: Props) => {
...
PdfCoproparasitologicoView.displayName = 'PdfCoproparasitologicoView';

export function ExamPreviewCoproparasitologico({ values }: Props) {
  const debouncedValues = useDebouncedValue(values, 3000);
  return <PdfCoproparasitologicoView values={debouncedValues} />;
}
