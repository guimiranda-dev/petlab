...
import PDFFile from '../PDFExam/urinalise';
...
const PdfUrinaliseView = memo(({ values }: Props) => {
...
PdfUrinaliseView.displayName = 'PdfUrinaliseView';

export function ExamPreviewUrinalise({ values }: Props) {
  const debouncedValues = useDebouncedValue(values, 3000);
  return <PdfUrinaliseView values={debouncedValues} />;
}
